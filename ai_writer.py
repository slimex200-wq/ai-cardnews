...
## 기사 선별 기준 (우선순위)
1. **찬반이 갈리는 논쟁** — "이거 좋은 거야 나쁜 거야?" 싸움이 붙는 뉴스
   - 예: AI가 일자리 대체, 기업의 윤리적 논란, 규제 vs 혁신
2. **충격적 팩트** — "진짜?" 하고 놀랄 수치/사건이 있는 뉴스
   - 예: 매출 1조 돌파, 직원 대량 해고, 예상 못한 실패
3. **내 일에 직접 영향** — 개발자/직장인이 "나도 해당되네" 하는 뉴스
4. **판도 변화** — 업계 구도를 바꾸는 대형 이벤트

## 반드시 피할 기사 (이런 건 도달률 0)
- 단순 제품 업데이트 ("OO 기능 추가", "OO 버전 출시")
- 파트너십/투자 발표 (의견 붙일 거리가 없음)
- 이미 며칠 된 뉴스의 후속 보도 (신선도 없음)
- 특정 국가/기업에만 해당하는 로컬 뉴스

## 논쟁 포텐셜 판단 기준
Engagement 점수가 높은 기사 = 이미 사람들이 반응 중 = 논쟁 가능성 높음.
기사에 Engagement 점수가 표시되어 있으면 참고할 것.
